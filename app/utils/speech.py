...
class PronunciationAssessment:
    def __init__(self, subscription_key, service_region, file,text):
        ...
        self.language = 'en-US'
        self.speech_recognizer = speechsdk.SpeechRecognizer(speech_config=self.speech_config, language=self.language, audio_config=self.audio_config)
    ...
    def process_results(self):
        reference_words = [w.strip(string.punctuation) for w in self.reference_text.lower().split()]
        if self.enable_miscue:
            diff = difflib.SequenceMatcher(None, reference_words, [x.word.lower() for x in self.recognized_words])
            final_words = []
        ...
        final_accuracy_scores = [word.accuracy_score for word in final_words if hasattr(word, 'accuracy_score')]
        accuracy_score = sum(final_accuracy_scores) / len(final_accuracy_scores) if final_accuracy_scores else 0
        fluency_score = sum([x * y for (x, y) in zip(self.fluency_scores, self.durations)]) / sum(self.durations) if self.durations else 0
        completeness_score = len([w for w in final_words if w.error_type == "None"]) / len(reference_words) * 100 if reference_words else 0
        completeness_score = min(completeness_score, 100)
        prosody_score = sum(self.prosody_scores) / len(self.prosody_scores) if self.prosody_scores else 0
